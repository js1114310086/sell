<template>
  <div>
    <sub-title :subTitle="'添加参数'" :isRefresh="false"></sub-title>
    <div class="body">
  ...
  mounted() {
    this.$http.post("merchantGoodsStyle/merchant_goods_type_list").then(res => {
      ...
      if (this.$route.query.id) {
        this.isAdd = false;
        this.$http
          .post("merchantGoodsParam/merchant_goods_type_by_id", {
    ...
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          if (this.isAdd) {
            this.addEdit(
              "merchantGoodsParam/merchant_goods_type_add",
              this.ruleForm
            );
          } else {
            this.ruleForm.id = this.$route.query.id;
            this.addEdit(
              "merchantGoodsParam/merchant_goods_type_update",
              this.ruleForm
            );
          }
        } else {
